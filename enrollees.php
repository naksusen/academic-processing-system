...
if (isset($_GET['progvalue']) && !empty($_GET['progvalue'])) {
    $whereClause .= " WHERE program = ?";
    $params[] = $_GET['progvalue'];
}

if (isset($_GET['yrsec']) && !empty($_GET['yrsec'])) {
    if ($whereClause) {
        $whereClause .= " AND year_lvl = ?";
    } else {
        $whereClause .= " WHERE year_lvl = ?";
    }
    $params[] = $_GET['yrsec'];
}

// Add sorting
$orderBy = " ORDER BY last_n ASC";
...
$sql = "SELECT * FROM enrollees" . $whereClause . $orderBy . " LIMIT $limitStart, $rowsPerPage";
...
<div class="flex justify-center mt-4 mb-4">
    <?php
    // Calculate total rows with same filters
    ...
    $queryParams = [];
    if (isset($_GET['progvalue']) && !empty($_GET['progvalue'])) {
        $queryParams[] = "progvalue=" . urlencode($_GET['progvalue']);
    }
    if (isset($_GET['yrsec']) && !empty($_GET['yrsec'])) {
        $queryParams[] = "yrsec=" . urlencode($_GET['yrsec']);
    }
    if (isset($_GET['column']) && !empty($_GET['column'])) {
        $queryParams[] = "column=" . urlencode($_GET['column']);
    }
    if (isset($_GET['order']) && !empty($_GET['order'])) {
        $queryParams[] = "order=" . urlencode($_GET['order']);
    }
    
    $queryString = implode('&', $queryParams);
    $linkPrefix = $queryString ? "enrollees.php?" . $queryString . "&" : "enrollees.php?";
    
    // Previous page button
    if ($currentPage > 1) {
        echo "<a href='$linkPrefix" . "page=" . ($currentPage - 1) . "' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-3xl'>Previous</a>";
    }

    // Page numbers
    for ($i = 1; $i <= $totalPages; $i++) {
        $activeClass = ($i == $currentPage) ? 'bg-orange-600' : 'bg-stone-500';
        echo "<a href='$linkPrefix" . "page=$i' class='mx-1 px-2 py-1 $activeClass text-white rounded-full'>$i</a>";
    }

    // Next page button
    if ($currentPage < $totalPages) {
        echo "<a href='$linkPrefix" . "page=" . ($currentPage + 1) . "' class='mx-2 px-4 py-2 bg-stone-500 text-white rounded-3xl'>Next</a>";
    }
    ?>
</div>
<!--PAGE BUTTON END-->
